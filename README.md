...
In unit tests it is sometimes useful to prevent the code from making real HTTP requests. Moa includes `MoaSimulator` class for testing image downloads. It helps verifying image downloads by faking network responses. See [unit test manual](https://github.com/evgenyneu/moa/wiki/Unit-testing-with-Moa) for more information.
